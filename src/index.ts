...
app.post("/api/v1/signin", async (req, res) => {
  ...
  const { username, password } = req.body;
  const user = await UserModel.findOne({ username }) as { password: string, _id: mongoose.Types.ObjectId }
  if (!user) {
    res.status(403).json({
...
app.get("/api/v1/content", userMiddleware, async (req, res) => {
  ...
  try {
    ...
    const content = await ContentModel.find({ userId: user }).populate("userId", "username")
    ...
    const transformedContent = await Promise.all(content.map(async (item) => {
      ...
      const tagIds: mongoose.Types.ObjectId[] = item.tags.map((tag: { _id: mongoose.Types.ObjectId }) => tag._id);
